...
from mirror_controller import Mirrors
...
class HexagonSensorVisualizer(QMainWindow):
    ...
    async def setup(self):
        # 后台开启传感器数据处理流程
        asyncio.create_task(self.mirror.run(self.sensor_data))
        # asyncio.create_task(self.update_sensor_data())
        asyncio.create_task(self.update_scatter_val())
    
    async def update_sensor_data(self):
        while True:
            Force, _ = await self.mirror.get_force()
            self.sensor_data = Force[0]
            await asyncio.sleep(1)    # 0.05秒刷新一次
    ...
    async def update_scatter_val(self):
        while True:
            for item in self.current_text_items:
                self.main_plot.removeItem(item)
            self.current_text_items = []
            ...
            for i, (x, y) in enumerate(self.sensor_positions): # i 就是传感器的物理位置
                ...
                self.main_plot.addItem(text)
                self.current_text_items.append(text)
            await asyncio.sleep(1)  # 0.05秒更新一次
